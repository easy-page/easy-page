...
export const syncFactorsForNewAct = ({
  ...
  console.log('firstDataCollector:', firstDataCollector)
  ...
  firstSubActChoosedFactors.forEach((each) => {
    ...
    console.log('eeerereererr:', factor)
...
const getNewQualify = (collector?: DataCollector) => {
  const newQualify: DataCollector = {}
  Object.keys(collector || {}).forEach((x) => {
    newQualify[x] = collector?.[x]
  })
  return newQualify
}
...
export const factorChangeEffects: (
  ...
  Object.values(formUtils || {}).map((e, idx) => {
    setTimeout(() => {
      ...
      /** 当前 subact 最新 qualify  */
      const newQualify = getNewQualify(curQualify?.dataCollector || {})

      /** 本子活动最新的 subact qualify  */
      ...
      latestSubActChoosedFactors.forEach((x) => {
        ...
        if (latestDataCollector?.[x] !== undefined) {
          newQualify[x] = latestDataCollector?.[x]
        } else {
          delete newQualify[x]
        }
      })

      console.log('1231231231232131:', newQualify, curQualify?.dataCollector)

      const curQualifyDataCollector = curQualify?.dataCollector || {}
      const curQualifyFactorCodes = Object.keys(curQualifyDataCollector)
      curQualifyFactorCodes.forEach((x) => {
        const curQualify = toFactorValue(curQualifyDataCollector?.[x])

        const isPoi =
          curQualify &&
          curQualify.feExtend?.factorCategoryCode === CategoryCode.Poi
        if (isPoi && !latestSubActChoosedFactors.includes(x)) {
          // 本次未选择，则删除
          delete newQualify[x]
        }
      })
