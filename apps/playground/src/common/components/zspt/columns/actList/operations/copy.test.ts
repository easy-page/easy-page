...
import { actCopy } from './copy'

const curOpItem = actCopy
...
describe('authOperationFromBackend', () => {
  ...
  it('should return allow: true if authInfo success and user not self and not authed', async () => {
    // 非授权、非本人，拒绝操作
    const spy = vi.spyOn(apis, 'getAuthInfo')
    spy.mockResolvedValueOnce({
      success: true,
