...
export const bgBuList = nodeUtil.createField<SelectState<ChargeSideEnum[]>>(
  'bgBuList',
  '补贴承担方',
  {
    mode: 'multiple',
    ...
    preprocess({ defaultValues }) {
      const choosed = defaultValues?.config?.bgBuList || []
    ...
    postprocess: ({ value }) => {
      return {
        'config.bgBuList': value.choosed ? [...value.choosed] : [],
      }
    },
    required: true,
  },
